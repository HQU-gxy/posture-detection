...
from app.utils import StyleFormatter
...
class Base(db.Model, Serializer):
  ...
  def init_query(cls,
                 params: dict,
                 query: Optional[Query] = None) -> Optional[Query]:
  ...
  def add(cls, data):
    ...
    return cls.add_model(
        model,
        print_warning_log=lambda e: logger.warning(
            f'{cls}: {e}\n INPUT JSON: {data}\n {model.serialize()}'),
        print_error_log=lambda e: logger.warning(
            f'{cls}: {e}\n INPUT JSON: {data}\n {model.serialize()}'))

  @classmethod
  def add_model(cls,
                model,
                print_warning_log: Optional[Callable[[str], None]] = None,
                print_error_log: Optional[Callable[[str], None]] = None):
    try:
  ...
  def filter_dict(cls, data: dict, exclude=[]):
    ...
    return {
        ...
        if k in cls.__table__.columns.keys() and k not in exclude and
  ...
  def filter_range_params(cls, data: dict, exclude=[]):
    ...
    return {
        k: v
        for k, v in data.items()
        if (k.startswith('min') or k.startswith('max') or
            k.startswith('start') or k.startswith('end')) and k not in exclude
    }

  @classmethod
  def range_query_snippet(cls, query: Query, params: dict):
    """
        为query拼接范围查询
