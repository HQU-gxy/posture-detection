...
def generate_final(excludes: list):
  empty: dict = YamlUtil.read_yaml("../app/doc/empty.yml")

  import os
  # 定义目录路径
  dir_path = os.path.normpath('../app/doc')
  ...
  YamlUtil.write_yaml("../app/doc/final.yml", empty)



from app.models import *
from app.utils.StyleFormatter import *
...
def generate_model(model: Base):
  ...
  for column in columns:
    ...
    model_data['properties'][snake_to_camel(column.name)] = {
        "type": convert_map[column.type.python_type],
...
def generate_model_and_write(model: Base, path: str):
  ...
  print(data)
  YamlUtil.write_yaml(f"{path}.yml", data)
